...
public class PlayerService implements PlayerRepository {
    ...
    public ArrayList<Player> getPlayers() {
        ...
        ArrayList<Player> list = new ArrayList<>(collection);
        return list;
    ...
    public Player updatePlayer(int playerId, Player player) {
        Player existedPlayer = team.get(playerId);
        if (existedPlayer == null) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND);
        }
        if (player.getPlayerName() != null) {
            existedPlayer.setPlayerName(player.getPlayerName());
        }
        if (player.getRole() != null) {
            existedPlayer.setRole(player.getRole());
        }
        if (player.getJerseyNumber() != 0) {
            existedPlayer.setJerseyNumber(player.getJerseyNumber());
        }
